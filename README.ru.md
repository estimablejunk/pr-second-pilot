...
**4. Настройка.** `/pr-second-pilot:setup` посмотрит на репозиторий и окружение и
предложит настройки — каждую с причиной. Ничего не пишет, пока не согласишься, и
показывает только то, что действительно надо менять.

Определяет, в частности, требуют ли тесты живых сервисов (тогда локальный гейт
ничего не проверяет и гейтом должен быть CI) и есть ли в коде авторизация или
платежи (тогда второй ревьюер окупается).

**5. Проверка.** `/pr-second-pilot:doctor` — покажет, что найдено и каким пулом
оплачивается ревью.
...
Полное имя обязательно. Плагин адресуется как `<плагин>@<маркетплейс>`, и здесь
оба совпадают — удвоение выглядит опечаткой, но ею не является. На короткую
форму команда отвечает «не найден».

В сессии то же самое доступно через `/plugin` → **Marketplaces →
pr-second-pilot → Update**. У сторонних маркетплейсов автообновление выключено
по умолчанию; включается там же.
...
/pr-second-pilot:setup                  первичная настройка с обоснованием
